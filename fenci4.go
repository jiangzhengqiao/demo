...
func main() {
	s := `本帖最后由 浪子理财 于 2012-2-11 23:23 编辑安全性，流动性和收益性是资本的三性，对网贷投资做简单的三性分析，抛砖引玉贻笑大方了。
 一，安全性原则。
 ...
 这种做法是对还是错呢？肯定没有错。安全性是基础，任何投资，安全永远是最重要的P2P理财。
 ...
 网贷网站日益增加的今天，哈哈贷倒了808信贷火了也不知道E速贷到底算不算p2p，贝尔创投不过是诈骗网站根本不能算是PPmoney，天使计划宛然升天1688易贷。我是风险偏好者，对于收益性比较看重，但是从投资角度讲，还是希望大家对风险多一份认识，风险中性者和风险规避者，还是少去杂七杂八的网站投资的好,陆金所到底搞不好。`
	// fmt.Println("原文：", s)
	// 一、加载词库
	x := gojieba.NewJieba("/Users/jiangzhengqiao/fenci4.txt")
	defer x.Free()
	...
	for e := res.Front(); nil != e; e = e.Next() {
		val := e.Value.(string)
		tag := x.Tag(val)[0]
		url := `<a href="` + tag[len(val)+2:len(tag)-1] + `">` + val + `</a>`
		s = strings.Replace(s, val, url, 1)
		// fmt.Println("关键字:", val)

	}
	fmt.Println("文本替换用时:", time.Now().Sub(t2))
...
func uniqueSlice(slice []string) (ret []string) {
	...
	for i, val := range slice {
		// 自动转换时间会浪费100µs
		// string(toLower([]byte(val))) == "p2p"
		if len(val) > 3 || val == "p2p" || val == "P2P" {
